...
interface Props {
  onAdd: (cfg: { name: string; color: string }) => void;
  onClose: () => void;
}

export function AddLinearSessionModal({ onAdd, onClose }: Props) {
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12, color: 'var(--text-hint)', lineHeight: 1.5 }}>
            A session runs tasks one after another with a single play/pause. Add tasks from inside the card.
          </p>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)' }}>Session name</label>
          <input
            autoFocus
            value={name}
